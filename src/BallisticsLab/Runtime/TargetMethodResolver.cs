...
namespace BallisticsLab.Runtime
{
    internal static class TargetMethodResolver
    {
            ...
            string name)
        {
            ...
            {
                throw new MissingMethodException("Required SPT 4.1.4 method was not found: " + name);
            }

            ParameterInfo[] actual = method.GetParameters();
            if (actual.Length != parameters.Length)
            {
                throw new MissingMethodException("Required SPT 4.1.4 signature changed: " + name);
            }
